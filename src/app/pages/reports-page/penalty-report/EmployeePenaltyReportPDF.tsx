...
const EmployeePenaltyReportPDF: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    let DepartmentID = lc.DepartmentID
    let EmployeeName = lc.EmployeeName
    let Day = lc.Day
  ...
  function getProjectPDF(
    ...
    if (ProjectID === 0) {
      URL = `${process.env.REACT_APP_API_URL}/GeneratePenalty/Download_Penalty_Report_By_EmployeeID_PDF`
      downloadLoader = true
    } else {
      URL = `${process.env.REACT_APP_API_URL}/GeneratePenalty/Download_Penalty_Report_By_EmployeeID_PDF`
      downloadLoader = true
    ...
    <>
      <div className='text-end '>
        <span
          ...
          onClick={() => {
            history.push({
              pathname: '/reports/employee-penalty/list',
              state: {
...
export default EmployeePenaltyReportPDF
