...
import {
  Get_Deadline_Report_By_TargateDate_EmpID_FilterAPI,
  Get_Deadline_Report_By_TargateDate_FilterAPI,
...
const ProjectDeadlineReportList: React.FC<Props> = () => {
  ...
  function getAllProjectDeadlineData(searchText: string, temDay: string) {
    if (user.roleID == 2 || user.roleID == 6) {
      Get_Deadline_Report_By_TargateDate_EmpID_FilterAPI(user.employeeID, temDay, searchText)
        .then((response) => {
      ...
      Get_Deadline_Report_By_TargateDate_FilterAPI(temDay, searchText)
        .then((response) => {
          let responseData = response.data
          let projectData = responseData.responseObject
          if (responseData.isSuccess == true) {
  ...
  return (
    <>
      ...
      <div className={`card`}>
        ...
        >
          <div className='col-xl-3 col-sm-6 mt-4 ms-2'>
            <Search
          ...
          <div className='position-absolute top-1 mt-2 end-0'>
            <Link
              to={{
                pathname: `/reports/project-deadline/download`,
                state: {
                  day: state.selDay,
                    ...
                    <tr className='border-bottom' key={index}>
                      ...
                      <td className='text-center'>
                        <td className='text-center'>
                          <Link
                            to={{
                              pathname: `/reports/project-deadline/view-details`,
                              state: {
...
export default ProjectDeadlineReportList
