...
const ProjectDeadlinePDF: React.FC<Props> = () => {
  ...
  function getProjectPDF(selDay: string, searchText: string, downloadLoader: boolean) {
    ...
    if (selDay === '') {
      URL = `${process.env.REACT_APP_API_URL}/AgencyWorkStage/Download_Deadline_Report_By_TargateDate_PDF`
      downloadLoader = true
    } else {
      URL = `${process.env.REACT_APP_API_URL}/AgencyWorkStage/Download_Deadline_Report_By_TargateDate_PDF`
      downloadLoader = true
    ...
    axios
      ...
      .then((blob) => {
        ...
        const fileName = 'Project_Deadline_Report_' + selDay + '_' + Tdate + '.pdf'
...
export default ProjectDeadlinePDF
