...
const StageWiseReportPDF: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getLedgerPdf(projectID, contactPerson)
    }, 100)
  }, [])

  function getLedgerPdf(projectID: number, contactPerson: string) {
    var URL = process.env.REACT_APP_API_URL
    if (parseInt(vendorID) === 0) {
      URL = `${process.env.REACT_APP_API_URL}/VendorReport/DownloadVendorProjectPaymentStructurePDFForAdmin`
    } else {
      URL = `${process.env.REACT_APP_API_URL}/VendorReport/DownloadVendorProjectPaymentStructurePDFForAdmin`
    }
    ...
    axios
      ...
      .then((blob) => {
        ...
        const fileName = 'StageWiseReport_' + contactPerson + '_' + Tdate + '.pdf'
...
export default StageWiseReportPDF
