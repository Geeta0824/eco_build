...
import {
  ...
  getVendorReportListByVendorIdApi,
...
const VendorReportList: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (vendorID == 0) {
        getVenderByVendorTypeIDData(vendorID)
      } else {
        getVenderByVendorTypeIDDataFromEye(vendorID)
      }
    }, 100)
  }, [])
    ...
    getVendorReportListByVendorIdApi(tmpVendortData.vendorID)
      .then((response) => {
        ...
        if (response.data.isSuccess == true) {
          ...
          setTotalVendor(vendorData.length)
          setPageVendor(1)
        } else {
  ...
  function getVenderByVendorTypeIDData(vendorID: number) {
    getVenderListByVendorTypeID(1)
      .then((response) => {
  ...
  function getVenderByVendorTypeIDDataFromEye(vendorID: number) {
    getVenderListByVendorTypeID(1)
      .then((response) => {
        const responseData = response.data.responseObject
        if (response.data.isSuccess == true) {
    ...
    ExportExcelVendorReportListApi(state.selVendorID)
      .then((response) => {
        if (response.data.isSuccess == true) {
          ...
          // XLSX.writeFile(workbook, `VendorReport_${moment(new Date()).format('YYYYMMDD')}.xlsx`)

          const workbook = new ExcelJS.Workbook()
          const worksheet = workbook.addWorksheet('VendorReport')

          // Define the columns with headers, keys, and manual widths
          worksheet.columns = [
            {header: 'Project Name', key: 'projectName', width: 35},
            {header: 'Project Category Name', key: 'projectCategoryName', width: 35},
            {header: 'Customer Name', key: 'customerName', width: 25},
            {header: 'Mobile Number', key: 'mobileNumber', width: 20},
            {header: 'Email', key: 'email', width: 30},
            {header: 'Vendor Cost', key: 'vendorCost', width: 18},
          ...
          workbook.xlsx.writeBuffer().then((buffer) => {
            ...
            link.download = `VendorReport_${moment(new Date()).format('YYYYMMDD')}.xlsx`
            link.click()
          })
        } else {
          toast.error(`${response.data.message}`)
        ...
        <div className='card-header border-0 row' style={{backgroundColor: '#000000'}}>
          <div className='mb-2 d-flex'>
            ...
            <div className='fv-row'>
              <div
                className='mt-3 btn btn-icon btn-bg-primary bg-hover-dark text-hover-inverse-dark btn-sm me-1 ms-6'
            ...
            <div className={state.selVendorID > 0 ? 'col-sm-6 mt-3 text-end' : 'd-none'}>
              <span
            ...
            <div className={state.selVendorID > 0 ? ' mt-3 text-end ms-5' : 'd-none'}>
              <Link
                to={{
                  pathname: `/account-reports/vendor/download/${state.selVendorID}`,
                  state: {
          ...
          <div className={state.selVendorID === 0 ? 'd-none' : 'row mb-2'}>
            ...
            <div className='col-4'>
              <label className='text-white fs-5'>Total Due Amount : &nbsp;</label>
            ...
            <table className='table table-bordered align-middle g-2'>
              ...
              <thead className='bg-light-primary'>
                <tr className='fw-bolder fs-6'>
                  ...
                  <th className='min-w-100px'>
                    <span className='d-block mb-1'>Amount Due</span>
                  </th>
                  <th className='min-w-150px'>
                    <span className='d-block mb-1 text-center'>View Stage Wise</span>
                  </th>
                  <th className='min-w-100px'>
                        ...
                        return (
                          <tr key={index}>
                              ...
                              {data.projectCategoryID === 1 ? (
                                <>
                                  {data.dueAmount > 0 ? (
                                    <Link
                                      to={{
                                        pathname: `/account-reports/vendor/amount-due-details`,
                                        state: {
                                          projectData: data,
                                          vendorID: state.selVendorID,
                                          companyName: state.selCompanyName,
                                ...
                                <>
                                  {data.dueAmount > 0 ? (
                                    <Link
                                      to={{
                                        pathname: `/account-reports/vendor/diy-amount-due-details`,
                                        state: {
                                          projectData: data,
                                          vendorID: state.selVendorID,
                                          companyName: state.selCompanyName,
                                ...
                                <>
                                  {data.dueAmount > 0 ? (
                                    <Link
                                      to={{
                                        pathname: `/account-reports/vendor/modular-amount-due-details`,
                                        state: {
                                          projectData: data,
                                          vendorID: state.selVendorID,
                                          companyName: state.selCompanyName,
                            ...
                            <td className='text-center'>
                              ...
                              <Link
                                to={{
                                  pathname: `/account-reports/vendor/stage-wise`,
                                  state: {
                                    projectData: data,
                                    vendorID: state.selVendorID,
                                    companyName: state.selCompanyName,
