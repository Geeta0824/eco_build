...
const OtherVendorPDF: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    const vendorID = lc.vendorID
    const companyName = lc.companyName
    setTimeout(() => {
      getLedgerPdf(companyName)
    }, 100)
  }, [])

  function getLedgerPdf(companyName: string) {
    var URL = process.env.REACT_APP_API_URL
    if (parseInt(vendorID) === 0) {
      URL = `${process.env.REACT_APP_API_URL}/VendorReport/Get_OtherVendorProjList_PDFForAdmin`
    } else {
      URL = `${process.env.REACT_APP_API_URL}/VendorReport/Get_OtherVendorProjList_PDFForAdmin`
    }
    ...
    axios
      ...
      .then((blob) => {
        ...
        const fileName = 'Vendor_' + companyName + '_' + Tdate + '.pdf'
