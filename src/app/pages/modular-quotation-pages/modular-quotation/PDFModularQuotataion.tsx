...
interface IPDFQuotation {
  ...
  mainCustomerID:number, mainEmployeeID:number, mainSearch:string
}

const PDFModularQuotataion: React.FC<Props> = () => {
  ...
  const [state, setState] = useState<IPDFQuotation>({
    loading: false,
    objpdf: null,
    mainCustomerID:0, mainEmployeeID:0, mainSearch:''
  })
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getQuotationPdf(mainCustomerID, mainEmployeeID, mainSearch)
    }, 100)
  }, [])

  function getQuotationPdf(mainCustomerID:number, mainEmployeeID:number, mainSearch:string) {
    var URL = ''
    if (location.pathname == `/modular/modular-quotation/pdf/${parseInt(quotationID)}`) {
      URL = `${process.env.REACT_APP_API_URL}/ModularPDF/DownloadModularQuotationPDF`
    } else if (
      location.pathname == `/modular/modular-quotation/agency-breakup-pdf/${parseInt(quotationID)}`
    ) {
      URL = `${process.env.REACT_APP_API_URL}/ModularPDF/DownloadModularQuotationPDFForAgencyBreackup`
    } else if (
      location.pathname == `/modular/modular-quotation/admin-pdf/${parseInt(quotationID)}`
    ) {
    ...
    axios.post(URL, {quotationID: parseInt(quotationID), isDownload: 0}).then((response) => {
      ...
      setState({...state,mainCustomerID, mainEmployeeID, mainSearch, loading: false, objpdf: tmpdv})
    })
  }
  ...
  return (
    <>
      <div className={state.loading === true ? 'd-none' : 'text-end'}>
        <span>
          {location.pathname === `/modular/modular-quotation/pdf/${parseInt(quotationID)}` ? (
            <Link
              ...
              to={{
                ...
                state: {modularTypeID: 0, employeeID: state.mainEmployeeID,
...
export default PDFModularQuotataion
