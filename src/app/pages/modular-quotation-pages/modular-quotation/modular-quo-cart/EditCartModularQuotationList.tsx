...
// import {saveQuotationDetailinCartApi} from '../../../../modules/quotations-master-page/diy-quotation-master-page/DIYQuotationCRUD'
import {toast} from 'react-toastify'
import Loader from '../../../common-pages/Loader'
import {useDispatch} from 'react-redux'
import {IModularProductListModel} from '../../../../models/modular-quotation-page/IModularQuotationModel'
import {UpdateModularQuotaionDetailApi, SaveModularQuotaionDetailInCartApi} from '../../../../modules/modular-quotation-master-page/diy-quotation-master-page/ModularQuotationCRUD'
import {IModularTypeModel} from '../../../../models/modular-product-page/modular-product-category/IModularProductCategoryModel'

type Props = {
  productList: IModularProductListModel[]
...
interface IAddCart {
  tmpProductList: IModularProductListModel[]
}
const EditCartModularQuotationList: React.FC<Props> = ({
  productList,
  modularTypeData,
  selQuotationId,
  tmpplanAreaID,
  ...
  const [state, setState] = useState<IAddCart>({
    tmpProductList: [] as IModularProductListModel[],
  })
  ...
  function handleLengthChange(event: any) {
    if (!isNaN(event.target.value)) {
      ...
      setValueLen(value)
      const Rows: IModularProductListModel[] = productList
      for (let key in Rows) {
  ...
  function onAddToCart(objProductList: IModularProductListModel) {
    // if (
    //   objProductList.modularTypeID == 0 ||
    //   tmpModularTypeID == undefined ||
    //   tmpModularTypeID == 0
    // ) {
    //   return toast.error(`Please Select Area.`, {autoClose: 1000})
    // }
    setMainLoading(true)
    SaveModularQuotaionDetailInCartApi(
      selQuotationId,
  ...
  function onEditToCart(objProductList: IModularProductListModel) {
    // if (
    //   objProductList.modularTypeID == 0 ||
    //   tmpModularTypeID == undefined ||
    //   tmpModularTypeID == 0
    ...
    // }
    setMainLoading(true)
    UpdateModularQuotaionDetailApi(
      tmpQuotationDetailsID,
      selQuotationId,
      objProductList.productCategoryID,
      objProductList.productID,
                    ...
                    <span className='text-gray-800 fw-bolder text-hover-primary fs-5'>
                      ...
                      >
                        ...
                        {modularTypeData.length > 0 &&
                          modularTypeData.map((moduTypeData, index) => {
                            return (
                              <option
                                key={index}
                                value={moduTypeData.modularTypeID}
                                selected={
                                  selAreaValue === moduTypeData.modularTypeID ? true : false
                                }
                              >
                                {moduTypeData.modularTypeName}
                              </option>
                            )
                          })}
                      </select>
...
export {EditCartModularQuotationList}
