...
interface IAddCart {
  ...
  objProductList: IModularProductListModel
...
const AddCartModularQuotationList: React.FC<Props> = ({
  selQuotationId,
  ...
  const [state, setState] = useState<IAddCart>({
    loading: false,
    productList: [] as IModularProductListModel[],
    modularTypeData: [] as IModularTypeModel[],
    tmpProductList: [] as IModularProductListModel[],
    objProductList: {} as IModularProductListModel,
  ...
  function handleLengthChange(event: any) {
    if (!isNaN(event.target.value)) {
      ...
      setValueLen(value)
      const Rows: IModularProductListModel[] = state.productList
      for (let key in Rows) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className='mt-5 col-xl-1 col-sm-4 d-flex align-content-around flex-wrap justify-content-center'>
        ...
        <span>
          <Link
            className='mt-6 btn btn-sm btn-light-primary bg-success text-white fs-5 mb-2 btn btn-rounded'
            to={{
        ...
        <div className='card-toolbar ms-1 mt-5'>
          {/* begin::Menu */}
          <button
            type='button'
...
export {AddCartModularQuotationList}
