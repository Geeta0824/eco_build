...
const CloneCarpetryPackage: React.FC = () => {
  ...
  const selectChange = (event: any) => {
    ...
    } else if (elementId === 'cloneTypeID') {
      formik.setFieldValue('cloneTypeID', parseInt(value))
      setState({...state, selCloneTypeID: parseInt(value)})
    }
  }

  const [photo, setPhoto] = useState('')
  const [fileLoader, setFileLoader] = useState(false)
  const imageUploadClone = (e: any) => {
    ...
    fetch(process.env.REACT_APP_API_URL + `/TurnkeyPackage/SaveTurnkeyPackageClonePhoto`, {
      method: 'POST',
      body: formData,
    })
      .then((res) => res.json())
        ...
        packageCloneTurnkeyPackageApi(
          ...
          .then((response) => {
            if (response.data.isSuccess == true) {
              ...
              history.push({
                pathname: `/carpetry/carpetry-pkg-mst/view-cart/${response.data.packageID}`,
                state: {
                  ...
                  packageAmount: resData.packageAmount,
                  cartListData: resData.responseObject,
                  photoPath: resData.photoPath,
                  searchText: state.mainSearch,
                  selProjectTypeID: resData.packageTypeID,
                  selCarpetAreaID: resData.carpetAreaID,
                  selBhkId: resData.bhkID,
                },
              })
              setLoading(false)
            } else {
          ...
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            <div className='card-body border-top p-9 ms-6'>
              ...
              <div className='row mb-6'>
                ...
                <label className='col-lg-2 col-form-label required fw-bold fs-6'>Clone Type:</label>
                <div className='col-lg-4 fv-row'>
                  <select
                    className='form-select bg-light-primary'
                    aria-label='Default select example'
                    ...
                    id='cloneTypeID'
                  >
                    <option selected={state.selCloneTypeID === 0 ? true : false} value={0}>
                      Select Clone Type
                    </option>
                    {state.ProjectTypeData.length > 0 &&
                      state.ProjectTypeData.map((data, index) => {
                        return (
                          <option
                            key={index}
                            value={data.projectTypeID}
                            selected={state.selCloneTypeID == data.projectTypeID ? true : false}
                          >
                            {data.projectType}
                          </option>
                        )
                  ...
                  {formik.touched.cloneTypeID && formik.errors.cloneTypeID && (
                    <div className='fv-plugins-message-container text-danger'>
                      <div className='fv-help-block'>{formik.errors.cloneTypeID}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className='row mb-6'>
                ...
                <div className='col-lg-4 fv-row'>
                  <input
                    ...
                    {...formik.getFieldProps('pricePerArea')}
                  />
                  {formik.touched.pricePerArea && formik.errors.pricePerArea && (
                    <div className='fv-plugins-message-container text-danger'>
                      <div className='fv-help-block'>{formik.errors.pricePerArea}</div>
                    </div>
                  )}
                </div>
              </div>
