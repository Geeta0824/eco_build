...
const PDFCarpetryQuotation: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getQuotationPdf(projectTypeID, mainEmployeeID, mainCustomerID, mainSearch)
    }, 100)
  }, [])
  ...
  function getQuotationPdf(
    ...
    var URL = ''
    if (location.pathname == `/quotations/ready-made-quotation/pdf/${parseInt(quotationID)}`) {
      URL = `${process.env.REACT_APP_API_URL}/TurnkeyPDF/Download_Carpentry_Quotaion_PDF`
    } else if (
      location.pathname == `/quotations/ready-made-quotation/admin-pdf/${parseInt(quotationID)}`
    ) {
      URL = `${process.env.REACT_APP_API_URL}/TurnkeyPDF/Download_Carpentry_Quotaion_ForAdmin_PDF`
    } else if (
      location.pathname ==
      `/quotations/ready-made-quotation/agency-berakup-pdf/${parseInt(quotationID)}`
    ) {
    ...
    axios
      .post(URL, {quotationID: parseInt(quotationID), isDownload: 0, employeeID: user.employeeID})
      .then((response) => {
...
export default PDFCarpetryQuotation
