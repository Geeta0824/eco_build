...
import {
  ...
  getMultiDropdownReadyMadeQuoApi,
...
const AddCarpetryQuotation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      // getBHKData(projectTypeID)
      getMultipleDropdownListData(projectTypeID, mainEmployeeID, mainCustomerID, mainSearch)
    }, 100)
  }, [])
    ...
    getMultiDropdownReadyMadeQuoApi()
      .then((response) => {
        ...
        if (response.data.isSuccess == true) {
          formik.setFieldValue('projectTypeID', selProjectTypeID)
          setState({
            ...state,
  ...
  // function getBHKData(selProjectTypeID: number) {
  //   getActiveBHKApi()
  //     .then((response) => {
  //       let responseData = response.data.responseObject
  //       getCarpetAreaData(selProjectTypeID, responseData)
  //     })
  //     .catch((error) => {
  //       toast.error(`${error}`)
  ...
  // function getCarpetAreaData(selProjectTypeID: number, bhkData: IBHKMasterModel[]) {
  //   getAllCarpetArea()
  //     .then((response) => {
  //       let responseData = response.data.responseObject
  //       getAllCustomerData(selProjectTypeID, bhkData, responseData)
  //     })
  //     .catch((error) => {
  //       toast.error(`${error}`)
  ...
  //       if (response.data.isSuccess === true) {
  //         getProjectTypeData(selProjectTypeID, bhkData, carpetAreaData, responseData)
  ...
  //   ProjectTypeData: IProjectTypeodel[]
  // ) {
  //   GetTurnkeyQuotationListApi(0, 0, 0, '')
  //     .then((response) => {
  //       if (response.data.isSuccess == true) {
  //         const responseData = response.data.responseObject
  //         setState({
          ...
          .then((response) => {
            if (response.data.isSuccess == true) {
              ...
              if (state.selQuotationID == 0) {
                ...
                history.push({
                  pathname: `/quotations/ready-made-quotation/view-cart/${response.data.quotationID}`,
                  state: {
                    packageID: resData.packageID,
                    packageTypeID: 1,
                    customerName: resData.customerName,
  ...
  return (
    <>
          ...
          <div id='kt_account_profile_details' className='collapse show'>
            <form onSubmit={formik.handleSubmit} noValidate className='form'>
              <div className='card-body border-top p-9 ms-6'>
                ...
                <div
                  className={state.selQuotationID === 0 ? 'd-none' : 'row pb-6 bg-light-warning'}
                >
                  <label className='col-lg-2 col-form-label fw-bold fs-6'>
      ...
      <Modal size='xl' scrollable={true} show={showClone} onHide={handleCloseClone}>
        <div style={{backgroundColor: '#2a3952'}}>
          <Modal.Header closeButton>
            <Modal.Title style={{color: 'white'}}>Ready Made Quotation Data</Modal.Title>
