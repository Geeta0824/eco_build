...
import {getAllDiscount} from '../../../modules/carpetry-master-page/carpetry-discount-master-page/CarpetryDiscountCRUD'
import {CarpetryDiscountCard} from './CarpetryDiscountCard'
...
interface IUnit {
  loading: boolean
  discountData: ICarpetryDiscountModel[]
  tmpDiscountData: ICarpetryDiscountModel[]
  SearchText: string
  selunitID: number
  activeID: number
  activeType: any
...
const CarpetryDiscountListPage: React.FC<Props> = () => {
  const location = useLocation()
  const [state, setState] = useState<IUnit>({
    loading: false,
    discountData: [] as ICarpetryDiscountModel[],
    tmpDiscountData: [] as ICarpetryDiscountModel[],
    SearchText: '',
    selunitID: 0,
    activeID: 0,
    activeType: false,
  ...
  const currentPosts: ICarpetryDiscountModel[] = state.discountData.slice(
    indexOfFirstPage,
    indexOfLastPage
  )
    ...
    <>
      <div className={`card `}>
        ...
        <Header_Search_Add
          searchText={name}
          filter={(e) => filter(e)}
          pathName={'/carpetry/discount/add'}
          title='Click to add a Carpetry Discount'
        />
        {/* <div className='card-header border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          >
            <Link to='/carpetry/discount/add' className='btn btn-sm btn-light-primary bg-white'>
              ...
              <tbody className="border-bottom">
                ...
                {currentPosts.length > 0 &&
                  currentPosts.map((data, index) => {
                    return (
                      <CarpetryDiscountCard data={data} name={name} />
                      // <tr key={index}>
                      //   <td>
                      ...
                      //       <Link
                      //         to={`/carpetry/discount/edit/${data.discountID}`}
...
export default CarpetryDiscountListPage
