...
import {
  GetFundReceiveByFundReceiveID,
  GetProjectInvoiceForFundRecByProjectInvoiceID,
  UpdateProjectFundReceive,
...
const EditFundReceive: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      let mainSearch: any = lc.mainSearch
  ...
  function getProjectDataByProjectID(
    ...
    GetFundReceiveByFundReceiveID(parseInt(projectPaymentID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          let temCashAccountTypeID = response.data.cashAccountID
          let temPaymentID = response.data.transactionModeID
          let temEmployeeBankID = response.data.cashAccountBankID
          let temProjectInvoiceID = response.data.projectInvoiceID
          let temProjectName = response.data.projectName
          let temProjectID = response.data.projectID
          formik.setFieldValue('projectName', response.data.projectName)
          formik.setFieldValue('projectID', response.data.projectID)
          formik.setFieldValue('projectInvoiceID', response.data.projectInvoiceID)
          ...
          setChequeAmount(response.data.chequeAmount)
          ...
          getAllProjectInvoiceListByProjectIDData(
            projectData,
            mainProjectID,
            mainStartTime,
            mainEndTime,
  ...
  // ===============Invoice Api ========================
  function getAllProjectInvoiceListByProjectIDData(
    projectData: IProjectModel[],
    mainProjectID: number,
    mainStartTime: string,
    mainEndTime: string,
    ...
    temProjectInvoiceID: number,
    temProjectName: string,
    temProjectID: number
  ) {
    GetProjectInvoiceForFundRecByProjectInvoiceID(temProjectInvoiceID)
      .then((response) => {
        if (response.data.isSuccess === true) {
          const responseData = response.data
          formik.setFieldValue('projectInvoiceID', responseData.projectInvoiceID)
          formik.setFieldValue('voucherNumber', responseData.invNumber)
          formik.setFieldValue('projectInvoiceAmount', responseData.invAmount)
          formik.setFieldValue('totalAmount', responseData.totalAmount)
          formik.setFieldValue('gstAmount', responseData.gstAmount)
          formik.setFieldValue('remainingAmount', responseData.remainingAmount)
          formik.setFieldValue('invoicePaidAmount', responseData.paidAmount)
          formik.setFieldValue('invoiceDate', responseData.invoiceDate)
          ...
          for (let key in Rows) {
            if (Rows[key].projectID === temProjectID) {
              ...
              formik.setFieldValue('customerName', Rows[key].firstName + ' ' + Rows[key].lastName)
              formik.setFieldValue('projectCategoryName', Rows[key].projectCategoryName)
              formik.setFieldValue('projectAmount', Rows[key].projectAmount)
              formik.setFieldValue('paidAmount', Rows[key].paidAmount)
              formik.setFieldValue('remainigAmt', Rows[key].remainingAmount)
          ...
          setState({
            ...
            selProjectInvoiceID: temProjectInvoiceID,
          ...
          {state.loading ? (
            ...
            <form onSubmit={formik.handleSubmit} noValidate className='form'>
              <div className='card-body border-top p-9 ms-6'>
                ...
                {/* <div className={state.selFundRecTypeID === 1 ? 'row mb-6 ' : 'd-none '}> */}
                <div className={'row mb-6'}>
                  <label className='col-lg-3 col-form-label fw-bold fs-6'>
...
export default EditFundReceive
