...
const AddGSTPayPage: React.FC = () => {
    ...
    GetCashAccountListAPI()
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          GetGSTBreakup(responseData, state.gstYear, state.gstMonth, mainSearch)
        } else {
  ...
  const selectChange = (event: any) => {
    ...
    if (elementId === 'cashAccountID') {
      ...
      // console.log(state.selCashAccountTypeID)
      // console.log(state.selTypeID)
      if (parseInt(value) === 0) {
        return
      } else {
      ...
      GetGSTBreakup(state.cashAccountData, parseInt(value), state.selMonthID, state.mainSearch)
    } else if (elementId === 'MonthID') {
      formik.setFieldValue('MonthID', parseInt(value))
      GetGSTBreakup(state.cashAccountData, state.selYearID, parseInt(value), state.mainSearch)
    }
  }
