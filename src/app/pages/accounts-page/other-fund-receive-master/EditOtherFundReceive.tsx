...
import {
  UpdateOtherFundReciveAPI,
  GetFranchiseListByFilterAPI,
  GetOtherFundReceiveByOtherPaymentIDAPI,
...
const EditOtherFundReceive: React.FC = () => {
    ...
    GetCashAccountListAPI()
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          GetOtherFundReceiveByOtherPaymentID(responseData, mainSearch)
        } else {
  ...
  function GetOtherFundReceiveByOtherPaymentID(
    cashAccountData: ICashAccountModel[],
    mainSearch: string
  ) {
    GetOtherFundReceiveByOtherPaymentIDAPI(parseInt(otherPaymentID))
      .then((response) => {
        const responseData = response.data
        if (responseData.isSuccess === true) {
          let temCashAccountTypeID = responseData.cashAccountID
          let temPaymentID = responseData.transactionModeID
          let temEmployeeBankID = responseData.cashAccountBankID
          let temPaymentFromID = responseData.paymentFromID
          let temFranchiseID = responseData.franchiseID
          let temVendorTypeID = responseData.vendorTypeID
          let temVendorID = responseData.vendorID
          formik.setFieldValue('payFromTypeID', responseData.paymentFromID)
          formik.setFieldValue('franchiseID', responseData.franchiseID)
          ...
          formik.setFieldValue('tcsPercentage', response.data.tcsPer)
          formik.setFieldValue('tcsAmount', response.data.tcsAmount)
          setQuotationFilePath(response.data.filePath)
          setAmount(response.data.amount)
          setIsTcs(response.data.isTCS)
          getVenderDataByVendorTypeID(
            cashAccountData,
            mainSearch,
            temCashAccountTypeID,
            temPaymentID,
            temEmployeeBankID,
            temPaymentFromID,
            temFranchiseID,
        ...
        setState({
          ...
          selPayFromTypeID: responseData.paymentFromID,
          loading: false,
        })
      })
      .catch((error) => {
  ...
  function getVenderDataByVendorTypeID(
    cashAccountData: ICashAccountModel[],
    mainSearch: string,
    temCashAccountTypeID: number,
    temPaymentID: number,
    temEmployeeBankID: number,
    temPaymentFromID: number,
    temFranchiseID: number,
    ...
    getVenderListByVendorTypeID(temVendorTypeID)
      .then((response) => {
        ...
        for (let key in Rows) {
          if (Rows[key].vendorID === temVendorID) {
            formik.setFieldValue('vendorID', Rows[key].vendorID)
            formik.setFieldValue('companyName', Rows[key].companyName)
            formik.setFieldValue('contactPerson', Rows[key].contactPerson)
            formik.setFieldValue('contactNumber', Rows[key].contactNumber)
            break
          }
        }
        if (response.data.isSuccess == true) {
  ...
  function GetFranchiseData(
    ...
    GetFranchiseListByFilterAPI(state.SearchText)
      .then((response) => {
        const responseData = response.data.responseObject
        const Rows = responseData
        for (let key in Rows) {
          if (Rows[key].employeeID === temFranchiseID) {
            formik.setFieldValue('employeeID', Rows[key].employeeID)
            formik.setFieldValue('firstName', Rows[key].firstName)
            formik.setFieldValue('lastName', Rows[key].lastName)
            formik.setFieldValue('branchName', Rows[key].branchName)
            formik.setFieldValue('email', Rows[key].email)
            formik.setFieldValue('contactNumber', Rows[key].contactNumber)
            formik.setFieldValue('franchiseName', Rows[key].firstName + ' ' + Rows[key].lastName)
            break
          }
        }
        if (response.data.isSuccess == true) {
          setState({
            ...
            selEmployeeBankID: temEmployeeBankID,
            selCashAccountTypeID: temCashAccountTypeID,
            selFranchiseID: temFranchiseID,
            selPayFromTypeID: temPaymentFromID,
            selPaymentID: temPaymentID,
            loading: false,
          })
        } else {
          toast.error(`${response.data.message}`)
...
export default EditOtherFundReceive
