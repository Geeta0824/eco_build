...
interface IDepartment {
  ...
  tmpVendorProjectData: IProjectModel[]
...
const AddPayFund: React.FC = () => {
  ...
  const [state, setState] = useState<IDepartment>({
    ...
    tmpVendorProjectData: [] as IProjectModel[],
  ...
  function onChangeDate(date: any) {
    ...
    let uid: number = date.target.id
    // let isChecked = e.target.checked
    let tmpProjData = state.projectData
    for (let k in tmpProjData) {
      if (uid == tmpProjData[k].projectVendorID) {
        tmpProjData[k].paymentDate = newDate
        break
      }
    }
    setState({...state, projectData: tmpProjData})
  ...
  function setInputAmt(e: any) {
    ...
    if (!isNaN(parseFloat(tmpValue)) && re.test(tmpValue)) {
      let tmpProjctData = [...state.projectData] // make sure you create a copy
      ...
      for (let k in tmpProjctData) {
        if (uid == tmpProjctData[k].projectVendorID) {
          ...
          amtTotal += parseFloat(tmpProjctData[k].amt) || 0
        }
      }
      ...
      for (let k in tmpProjctData) {
        ...
        amtTotal += parseFloat(tmpProjctData[k].amt) || 0 // recalculate total
      }
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        if (values.projectModeID == 1) {
          ...
          let tmplstCheckedOutputData = [] as IFundProjectModel[]
          let resultOptputObj: IProjectModel[] = state.projectData
          for (let k in resultOptputObj) {
            if (resultOptputObj[k].isSelected == 1) {
              let tmpCheckedData: IFundProjectModel = {
                projectID: resultOptputObj[k]['projectID'],
                amount: resultOptputObj[k]['amt'],
                projectVendorID: resultOptputObj[k]['projectVendorID'],
                paymentDate: resultOptputObj[k]['paymentDate'],
              }
                ...
                <tbody className="border-bottom">
                  {currentPosts.length > 0 &&
                    currentPosts.map((data, index) => {
                      return (
                        <ModelProjectList data={data} selectProject={() => selectProject(data)} />
                      )
                    })}
                  <BlankDataImageInTable
