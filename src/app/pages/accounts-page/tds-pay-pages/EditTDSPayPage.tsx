...
const EditTDSPayPage: React.FC = () => {
  const [data, setData] = useState<ITDSPayModel>(initialValues)
  const [filePath, setFilePath] = useState<string>('')
  const [loading, setLoading] = useState(false)
  const {tdsPayID} = useParams<{tdsPayID: string}>()
    ...
    GetCashAccountListAPI()
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          getTDSDataByTDSPaymentID(responseData, mainSearch)
        } else {
  ...
  function getTDSDataByTDSPaymentID(cashAccountData: ICashAccountModel[], mainSearch: string) {
    GetTDSByTDSPaymentIDAPI(parseInt(tdsPayID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          ...
          let temMonthID = response.data.tdsMonth
          let temYearID = response.data.tdsYear
          ...
          formik.setFieldValue('referenceNo', response.data.referenceNo)
          ...
          formik.setFieldValue('paymentDate', response.data.tdsPayDate)
          formik.setFieldValue('MonthID', response.data.tdsMonth)
          formik.setFieldValue('YearID', response.data.tdsYear)
          setAmount(response.data.tdsAmount)
          setFilePath(response.data.documentPath)
          GetTDSBreakupDataList(
            cashAccountData,
            mainSearch,
            temCasAccID,
            temCasAccBankID,
  ...
  function GetTDSBreakupDataList(
    cashAccountData: ICashAccountModel[],
    mainSearch: string,
    temCasAccID: number,
    ...
    GetTDSBreakupData(temYearID, temMonthID)
      .then((response) => {
  ...
  function GetTDSBreakup(tdsYear: number, tdsMonth: number) {
    GetTDSBreakupData(tdsYear, tdsMonth)
      .then((response) => {
        const responseData = response.data.responseObject
        if (response.data.isSuccess === true) {
          setAmount(response.data.tdsTotalAmount)
          setState({...state, TDSBreakupData: responseData, loading: false})
        } else {
          toast.error(`${response.data.message}`)
          setState({...state, TDSBreakupData: [], loading: false})
        }
  ...
  const selectChange = (event: any) => {
    ...
    if (elementId === 'cashAccountID') {
      ...
      GetTDSBreakup(parseInt(value), state.selMonthID)
    } else if (elementId === 'MonthID') {
      formik.setFieldValue('MonthID', parseInt(value))
      GetTDSBreakup(state.selYearID, parseInt(value))
    }
  }
