...
import {AddAccountTransferDetails} from '../../../modules/account-page/account-transfer-master-page/AccountTransferCRUD'
...
interface IDepartment {
  ...
  selcashTypeID: number
  departmentData: IAccountTransferModel[]
...
const AddAccountTransfer: React.FC = () => {
  ...
  const [state, setState] = useState<IDepartment>({
    ...
    selToEmpId: 0,
    selFromEmpId: 0,
    departmentData: [] as IAccountTransferModel[],
      ...
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          setState({
            ...
            FromCashAccountID: selFromCashAccountID,
            ToCashAccountID: selToCashAccountID,
            StartDate,
            EndDate,
            SearchText,
            loading: false,
  ...
  const selectChange = (event: any) => {
    ...
    } else if (elementId === 'ToSubaccontID') {
      formik.setFieldValue('ToSubaccontID', parseInt(value))
  ...
  const formik = useFormik<IAccountTransferModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        AddAccountTransferDetails(
          values.transferDate,
          values.amount,
          values.fromAccountID,
          values.fromSubAccountID,
          values.toAccountID,
          values.ToSubaccontID,
        ...
        <div id='kt_account_profile_details' className='collapse show'>
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            <div className='card-body border-top p-9 ms-6'>
                      ...
                      state.subAccountFromData.map((data, index) => {
                        return (
                          <option
                            ...
                            selected={
                              data.cashEmployeeBalanceID == state.selFromSubAccountID ? true : false
                            }
                          >
              ...
              <div className='row mb-6'>
                <label className='col-lg-2 col-form-label required fw-bold fs-6'>To Account:</label>
                <div className='col-lg-4 fv-row'>
                  <select
                    className='form-select bg-light-primary'
                    aria-label='Default select example'
                      ...
                      state.cashAccountData.map((data, index) => {
                        return (
                          <option
                            ...
                            id={`${data.cashAccountTypeID}`}
                            lang={`${data.employeeID}`}
                            selected={data.cashAccounID === state.selToCashAccountID ? true : false}
                          >
                            {data.accountName}
                          </option>
                        )
                ...
                <div className={state.selToTypeID === 2 ? 'col-lg-4 fv-row' : 'd-none'}>
                  ...
                  {formik.touched.ToSubaccontID && formik.errors.ToSubaccontID && (
                    <div className='fv-plugins-message-container text-danger'>
                      <div className='fv-help-block'>{formik.errors.ToSubaccontID}</div>
                    </div>
                  )}
                </div>
              </div>
