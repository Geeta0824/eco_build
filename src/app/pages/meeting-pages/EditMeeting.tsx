...
interface IDepartment {
  ...
  agencyVendData: IAgencyListModel[]
  tmpAgencyVendData: IAgencyListModel[]
...
const EditMeeting: React.FC = () => {
  ...
  const [state, setState] = useState<IDepartment>({
    ...
    agencyVendData: [] as IAgencyListModel[],
    tmpAgencyVendData: [] as IAgencyListModel[],
  ...
  const selectChange = (event: any) => {
    ...
    if (elementId === 'venueID') {
      ...
      formik.setFieldValue('vendorID', parseInt(value)) // Update formik value
      setState({...state, selVendorID: parseInt(value)}) // Update state with the selected vendorID
    }
  }
  ...
  async function GetMeetingByMeetingIDData() {
    try {
      const response = await GetMeetingByMeetingID(parseInt(meetingID))
      const responseData = response.data.meetings[0]
      if (response.data.isSuccess) {
        formik.setFieldValue('vendorID', responseData.vendorID)
        formik.setFieldValue('venueID', responseData.venueID)
        formik.setFieldValue('statusID', responseData.statusID)
        formik.setFieldValue('projectID', responseData.projectID)
        formik.setFieldValue('mediumID', responseData.mediumID)
        formik.setFieldValue('description', responseData.description)
        formik.setFieldValue('startTime', responseData.startTime)
        formik.setFieldValue('endTime', responseData.endTime)
        formik.setFieldValue('meetingDate', responseData.meetingDate)
        formik.setFieldValue('employeeName', responseData.employeeName)
        setIsAgency(responseData.isAgency)
        setIsClient(responseData.isClient)
        await getAllProjectData(
          responseData.statusID,
          responseData.meetingTypeID,
          responseData.mediumID,
          responseData.venueID,
          responseData.projectID,
          responseData.vendorID,
          responseData.employeeIDs
        )
        // GetEmployeeMapListWithProjectData(responseData.projectID)
      } else {
  ...
  async function getAllProjectData(
    ...
    if (state.tmpProjectData.length > 0) return

    try {
      const response = await getAllProjectListByRoleIDAndEmployeeIDAPI(user.roleID, user.employeeID)
      const responseData = response.data.responseObject
      if (response.data.isSuccess) {
        for (let key in responseData) {
          if (responseData[key].projectID === projectID) {
            formik.setFieldValue('projectID', responseData[key].projectID)
            formik.setFieldValue('projectName', responseData[key].projectName)
            formik.setFieldValue('customerName', responseData[key].firstName)
            formik.setFieldValue('projectCategoryName', responseData[key].projectCategoryName)
            formik.setFieldValue('projectType', responseData[key].projectType)
            formik.setFieldValue('projectType', responseData[key].projectType)
            break
          }
        }
  ...
  async function GetMeetingTypeListData(
    projectData: IProjectModel[],
    statusID: number,
    meetingTypeID: number,
    mediumID: number,
    ...
    try {
      ...
      const meetingTypeData = response.data.responseObject

      if (response.data.isSuccess) {
        await GetMeetingStatusListData(
          projectData,
          meetingTypeData,
          statusID,
          meetingTypeID,
  ...
  async function GetMeetingStatusListData(
    projectData: IProjectModel[],
    meetingTypeDdData: IMeetingTypeDd[],
    statusID: number,
    meetingTypeID: number,
    ...
    try {
      ...
      const statusData = response.data.responseObject

      if (response.data.isSuccess) {
        await GetMeetingMediumListData(
          projectData,
          meetingTypeDdData,
          statusData,
          statusID,
  ...
  async function GetMeetingMediumListData(
    projectData: IProjectModel[],
    meetingTypeDdData: IMeetingTypeDd[],
    statusDDL: IStatusModelDD[],
    statusID: number,
    ...
    try {
      const response = await GetMeetingMediumList()
      const mediumData = response.data.responseObject

      if (response.data.isSuccess) {
        await GetMeetingVenueListData(
          projectData,
          meetingTypeDdData,
          statusDDL,
          mediumData,
  ...
  async function GetMeetingVenueListData(
    projectData: IProjectModel[],
    meetingTypeDdData: IMeetingTypeDd[],
    statusDDL: IStatusModelDD[],
    mediumsDDL: IMediumModelDD[],
    ...
    try {
      const response = await GetMeetingVenueList()
      const venueData = response.data.responseObject
  ...
  function GetEmpListWithProjectData(
    projectData: IProjectModel[],
    meetingTypeDdData: IMeetingTypeDd[],
    statusDDL: IStatusModelDD[],
    mediumsDDL: IMediumModelDD[],
      ...
      .then((response) => {
        ...
        if (response.data.isSuccess) {
          for (let item of responseData) {
            item.isMember = employeeIDs.includes(item.employeeID) ? 1 : 0
          }

          const preSelectedNames = responseData
            .filter((employee: any) => employee.isMember === 1)
            .map((employee: any) => employee.employeeName)

          setSelectedEmployeeNames(preSelectedNames)
          ...
          setState({
            ...
            mediumsDDL,
            venueDDL,
            selStatusID: statusID,
            selMeetingTypeID: meetingTypeID,
            selmediumID: mediumID,
            selVenueID: venueID,
  ...
  function selectProject(tmpProjectData: IProjectModel) {
    ...
    // GetVendorMapListWithProjectDataa(tmpProjectData.projectID)
    setShow(false)
  }
  // =================-------------------------- Pagination Project--------------------------===================
 ...
 function GetEmployeeMapListWithProjectData(projectID: number) {
  ...
  async function SetStatus(e: any) {
    ...
    // Update the isMember status for the clicked employee
              ...
              {/* <div className={isAgency === true ? 'row mb-6' : 'd-none'}>
                ...
                <div className='col-lg-4 fv-row'>
                  <select
                    ...
                    id='vendorID'
                    value={state.selProjectID} // Set the value of the select to the selected vendorID
                  >
                    <option value={0}>Select Agency</option>
                    {state.agencyData.length > 0 &&
                      state.agencyData.map((data, index) => (
                        <option key={index} value={data.vendorID}>
                          {data.companyName}
                        </option>
                      ))}
                  </select>
  ...
  <div className={isAgency === true ? 'row mb-6' : 'd-none'}>
                      ...
                      state.agencyData.map((data, index) => {
                        return (
                          <option
                            key={index}
                            value={data.vendorID}
                            selected={state.selVendorID == data.vendorID ? true : false}
                          >
                            {data.companyName}
                          </option>
                        )
              ...
              <div className='row mb-6'>
                <label className='col-lg-2 col-form-label fw-bold fs-6'>
                  <span className=''>Meeting Purpose:</span>
                </label>
