  ...
  const formik = useFormik<ICarpetryDiscountModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        if (Edit) {
          AddDiscountAPI(values.discountPercentage, values.branchID)
            .then((response) => {
              if (response.data.isSuccess === true) {
                toast.success('Create Successfull')
                history.push({pathname:'/master/discount/list',state:{search:state.mainSearch}})
                setLoading(false)
              } else {
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname:'/master/discount/list',state:{search:state.mainSearch}}}>
            Back To List
          </Link>
        </span>
      </div>
        ...
        <div id='kt_account_profile_details' className='collapse show'>
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            ...
            <div className='card-footer d-flex justify-content-end py-6 px-9'>
              ...
              <Link className='btn btn-danger' to={{pathname:'/master/discount/list',state:{search:state.mainSearch}}}>
                Cancel
              </Link>
            </div>
          </form>
