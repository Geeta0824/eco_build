...
import {
  ...
  getProjectListByAgencyIdCloseApi,
...
const CloseProjectListPage: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getCloseAgencyListByVendorID(agencyTypeName, agencyName, companyName)
    }, 100)
  }, [])

  function getCloseAgencyListByVendorID(agencyTypeName: string, agencyName: string, companyName: string) {
    getProjectListByAgencyIdCloseApi(parseInt(vendorID))
      .then((response) => {
    ...
    <> <Loader loading={state.loading} />
      <div className={`card `}>
        ...
        <div className='card-header row border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          <div className='text-start row mb-2 col-xl-3 mt-4 col-sm-6'>
                  ...
                  state.agencyOpenListData.map((data, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td className='text-dark text-hover-primary fs-6 mb-1'>{data.projectStatusName}</td>
                      </tr>
                    )
                  })}
