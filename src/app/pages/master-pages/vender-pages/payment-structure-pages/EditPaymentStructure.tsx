...
import {
  ...
  getPMCWorkStagePayStructureMapByPMCWorkStageMapID,
  updateVenderPaymentStructureMapDetails,
...
const EditPaymentStructure: React.FC = () => {
  ...
  const {payStrID} = useParams<{payStrID: string}>()
  ...
  useEffect(() => {
    setTimeout(() => {
      getPaymentStructureDataByID()
    }, 100)
  }, [])

  function getPaymentStructureDataByID() {
    getPMCWorkStagePayStructureMapByPMCWorkStageMapID(parseInt(payStrID))
      .then((response) => {
        const resData = response.data
        formik.setFieldValue('sequenceNo', resData.sequenceNo)
        formik.setFieldValue('stageName', resData.stageName)
        formik.setFieldValue('amtPercentage', resData.amtPercentage)
      })
      .catch((error) => {
        toast.error(`${error}`)
      })
