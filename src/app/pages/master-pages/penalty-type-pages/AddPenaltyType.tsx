...
import {AddPenaltyTypeAPI} from '../../../modules/master-page/Penalty-Type-page/PenaltyTypeCRUD'

const profileDetailsSchema = Yup.object().shape({
  penaltyTypeName: Yup.string().required('Penalty Type is required'),
})

const AddPenaltyType: React.FC = () => {
  ...
  const formik = useFormik<PenaltyTypeModel>({
    ...
    onSubmit: (values) => {
      setLoading(true)
      setTimeout(() => {
        AddPenaltyTypeAPI(values.penaltyTypeName, user.employeeID, '192.168.0.1')
          .then((response) => {
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname: '/master/penalty-type/list', state: {search: mainSearch}}}
          >
            Back To List
          </Link>
        </span>
