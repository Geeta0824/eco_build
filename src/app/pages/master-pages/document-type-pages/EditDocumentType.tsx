...
const EditDocumentType: React.FC = () => {
  ...
  const {doctypeId} = useParams<{doctypeId: string}>()
  ...
  const [state, setState] = useState<IDoc>({
    loading: false,
    mainSearch: '',
  })
  ...
  function getDocumentData(mainSearch: string) {
    let value = {documentTypeID: doctypeId}
    var objDocumentType = btoa(JSON.stringify(value))
    getUpdateDocument(`${objDocumentType}`)
      .then((response) => {
        var decodeResp = JSON.parse(atob(response.data.encodedResponse))
        let resp = decodeResp
        if (resp.isSuccess == true) {
          formik.setFieldValue('documentTypeID', resp.documentTypeID)
          formik.setFieldValue('documentTypeName', resp.documentTypeName)
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        if (edit) {
          ...
          var objDocumentType = btoa(JSON.stringify(updatedValues))
          updateDocumentType(`${objDocumentType}`)
            .then((response) => {
              var decodeResp = JSON.parse(atob(response.data.encodedResponse))
              let resp = decodeResp
              if (resp.isSuccess === true) {
                ...
                history.push({
                  pathname: '/master/documenttype/list',
    ...
    <>
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname: '/master/documenttype/list', state: {search: state.mainSearch}}}
          >
            Back To List
          </Link>
        </span>
      ...
      >
        <div className='card-title m-0'>
          <h3 className='fw-bolder m-0 ms-6'>Update DocumentType </h3>
        </div>
      </div> */}
