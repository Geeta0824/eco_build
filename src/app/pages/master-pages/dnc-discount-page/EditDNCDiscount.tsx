...
import {
  IDNCDescountModel,
  DIYdiscountInitValue as initialValues,
} from '../../../models/master-page/IDNCDescountModel'
import {
  getDiyDiscountByDiscountId,
  updateDiyDiscount,
} from '../../../modules/master-page/dnc-discount-master-page/DNCDiscountCRUD'
...
const EditDNCDiscount: React.FC = () => {
  const {discountID} = useParams<{discountID: string}>()
  const history = useHistory()
  const location = useLocation()
  const [data, setData] = useState<IDNCDescountModel>(initialValues)
  const updateData = (fieldsToUpdate: Partial<IDNCDescountModel>): void => {
    ...
    getBranchDropdownList()
      .then((response) => {
        ...
        if (resp.isSuccess === true) {
          ...
          getDiyDiscountDataByDiscountId(responseData, mainSearch)
        } else {
  ...
  function getDiyDiscountDataByDiscountId(branchData: IBranchModel[], mainSearch: string) {
    getDiyDiscountByDiscountId(discountID)
      .then((response) => {
  ...
  const formik = useFormik<IDNCDescountModel>({
    initialValues,
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        if (Edit) {
          updateDiyDiscount(parseInt(discountID), values.discountPercentage, values.branchID)
            .then((response) => {
              if (response.data.isSuccess === true) {
                toast.success('Edit Successfull')
                history.push({
                  pathname: '/master/dnc-discount/list',
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname: '/master/dnc-discount/list', state: {search: state.mainSearch}}}
          >
            Back To List
          </Link>
        </span>
