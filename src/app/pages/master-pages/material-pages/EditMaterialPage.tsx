...
import {
  getMaterialInfoDataByMaterialInfoID,
  updateMaterialInfoApi,
...
const EditMaterialPage: React.FC = () => {
  ...
  const {materialInfoID} = useParams<{materialInfoID: string}>()
    ...
    GetProjectTypeDropdownListAPI()
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          materialInfoDataByMaterialInfoID(responseData, mainSearch)
        } else {
  ...
  function materialInfoDataByMaterialInfoID(
    projectTypeData: IProjectTypeodel[],
    mainSearch: string
  ) {
    getMaterialInfoDataByMaterialInfoID(parseInt(materialInfoID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          formik.setFieldValue('projectTypeID', response.data.projectTypeID)
          formik.setFieldValue('materialName', response.data.materialName)
          formik.setFieldValue('doneby', response.data.doneby)
          formik.setFieldValue('specification', response.data.specification)
          formik.setFieldValue('importantPoint', response.data.importantPoint)
          setState({
            ...state,
        ...
        updateMaterialInfoApi(
          ...
          .then((response) => {
            if (response.data.isSuccess == true) {
              ...
              history.push({pathname: '/master/material/list', state: {search: state.mainSearch}})
              setLoading(false)
            } else {
              toast.error(`${response.data.message}`)
              setLoading(false)
