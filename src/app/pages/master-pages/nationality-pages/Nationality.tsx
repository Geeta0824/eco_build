...
interface INationality {
  ...
  tmpNationalityData: INationalityWebModel[]
  SearchText: string
  selNatioId: number
  activeID: number
  activeType: any
...
const Nationality: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getAllNationalityData(mainSearch)
    }, 100)
  }, [])

  function getAllNationalityData(mainSearch: string) {
    getAllNationality()
      .then((response) => {
          ...
          if (mainSearch !== '') {
            const results = responseData.filter((user: any) => {
              return (
                ...
                user.nationalityName.toLowerCase().includes(mainSearch.toLowerCase())
              )
              // Use the toLowerCase() method to make it case-insensitive
            })
            setState({
              ...state,
              nationalityData: results,
              tmpNationalityData: responseData,
              loading: false,
            })
  ...
  function checkedFunction(temEmpId: number, temIsAct: boolean) {
    isActiveNationality(temEmpId, temIsAct)
      .then((response) => {
        if (response.data.isSuccess == true) {
          getAllNationalityData(state.SearchText)
          setShowActive(false)
        } else {
          toast.error(`${response.data.message}`)
          setShowActive(false)
  ...
  const handleShow = (nationalityID: number) => {
    setState({
      ...state,
      selNatioId: nationalityID,
      loading: false,
    })
    setShow(true)
  }

  function deleteNatinalityItem(temNatId: number) {
    deleteNationality(temNatId)
      .then((response) => {
        if (response.data.isSuccess == true) {
  ...
  const filter = (e: any) => {
    ...
    if (keyword !== '') {
      const results = state.tmpNationalityData.filter((user) => {
        return (
          // user.countryName.toLowerCase().includes(keyword.toLowerCase()) ||
          user.nationalityName.toLowerCase().includes(keyword.toLowerCase())
        )
        // Use the toLowerCase() method to make it case-insensitive
      })
      setState({...state, nationalityData: results})
      setTotal(results.length)
      setPage(1)
    } else {
      setState({...state, nationalityData: state.tmpNationalityData})
      // If the text field is empty, show all users
      setTotal(state.tmpNationalityData.length)
      setPage(1)
    }
    setName(keyword)
  }
  ...
  const [total, setTotal] = useState(state.nationalityData.length) //  length
  const [page, setPage] = useState(1)
  const [postPerPage, setPostPerPage] = useState(10)
  const indexOfLastPage = page * postPerPage
  const indexOfFirstPage = indexOfLastPage - postPerPage
  const currentPosts: INationalityWebModel[] = state.nationalityData.slice(
    indexOfFirstPage,
    indexOfLastPage
  )
    ...
    <>
      <div className={`card `}>
        ...
        <Header_Search_Add
          searchText={name}
          filter={(e) => filter(e)}
          pathName={'/master/nationality/add'}
          title='Click to add a Nationality'
        />
        {/* <div className='card-header border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          >
            <Link to='/master/nationality/add' className='btn btn-sm btn-light-primary bg-white'>
            ...
            <table className='table table-bordered align-middle g-2'>
              ...
              <thead className='bg-light-primary'>
                <tr className='fw-bolder fs-5'>
                  <th className='min-w-150px'>Nationality Name</th>
                  ...
                  currentPosts.map((data, index) => {
                    return (
                      <NationalityCard
                        ...
                        handleShow={() => handleShow(data.nationalityID)}
                        name={name}
                      />
                      // <tr key={index}>
                      //   <td>
                      ...
                      //       <Link
                      //         to={`/master/nationality/edit/${data.nationalityID}`}
      ...
      <ModelPopUpDelete
        ...
        deleteData={() => deleteNatinalityItem(state.selNatioId)}
      />
      {/* ===================Is Active Model===================== */}
      <ModelPopUpIsActive
