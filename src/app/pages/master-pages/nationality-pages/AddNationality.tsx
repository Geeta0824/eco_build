...
import {createNationality} from '../../../modules/master-page/nationality-master-page/NationalityCRUD'
...
const AddNationality: React.FC = () => {
  ...
  const formik = useFormik<INationalityWebModel>({
    ...
    onSubmit: (values) => {
      setLoading(true)
      setTimeout(() => {
        createNationality(values.nationalityName, isActive, user.employeeID, '192.66.22')
          .then((response) => {
            if (response.data.isSuccess == true) {
              toast.success('Created Successfull')
              history.push({pathname: '/master/nationality/list', state: {search: mainSearch}})
              setLoading(false)
            } else {
    ...
    <>
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname: '/master/nationality/list', state: {search: mainSearch}}}
          >
            Back To List
          </Link>
        </span>
      ...
      >
        <div className='card-title m-0'>
          <h3 className='fw-bolder m-0 ms-6'>Create Nationality </h3>
        </div>
      </div> */}
