...
import {
  GetMeetingTypeByMeetingTypeID,
  UpdateMeetingTypeApi,
} from '../../../modules/master-page/meeting-type-page/MeetingTypeCRUD'
...
const EditMeetingType: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getMeetingTypeByMeetingTypeID(SearchText)
    }, 100)
  }, [])

  function getMeetingTypeByMeetingTypeID(SearchText: string) {
    GetMeetingTypeByMeetingTypeID(parseInt(meetingTypeid)).then((response) => {
  ...
  const formik = useFormik<IMeetingTypeModel>({
    ...
    onSubmit: (values) => {
      setLoading(true)
      setTimeout(() => {
        UpdateMeetingTypeApi(parseInt(meetingTypeid), values.meetingTypeName)
          .then((response) => {
