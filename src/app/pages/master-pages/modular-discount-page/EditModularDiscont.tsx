...
import {
  IDiscountModel,
  discountInitValue as initialValues,
} from '../../../models/master-page/IDiscountModel '
...
import { getModularDiscountByDiscountId, updateModularDiscount } from '../../../modules/master-page/modular-discount-master-page/ModularDiscountCRUD'
...
const EditModularDiscont: React.FC = () => {
  const {discountId} = useParams<{discountId: string}>()
  const history = useHistory()
  const location = useLocation()
  const [data, setData] = useState<IDiscountModel>(initialValues)
  const updateData = (fieldsToUpdate: Partial<IDiscountModel>): void => {
      ...
      .then((response) => {
        ...
        if (resp.isSuccess === true) {
          let responseData = resp.responseObject
            getModularDiscountDataByDiscountId(responseData,mainSearch)
        } else {
  ...
  function getModularDiscountDataByDiscountId(branchData: IBranchModel[],mainSearch: string) {
    getModularDiscountByDiscountId(discountId)
      .then((response) => {
        if (response.data.isSuccess == true) {
          ...
          setState({...state, branchData: [], selBranchID: 0, loading: false})
        }
      })
      .catch((error) => {
        toast.error(`${error}`)
  ...
  const formik = useFormik<IDiscountModel>({
    initialValues,
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        ...
        if (Edit) {
            updateModularDiscount(parseInt(discountId), values.discountPercentage, values.branchID)
            .then((response) => {
              if (response.data.isSuccess === true) {
                toast.success('Edit Successfull')
                history.push({pathname:'/master/modular-discount/list',state:{search:state.mainSearch}})
                setLoading(false)
              } else {
                toast.error(`${response.data.message}`)
                setLoading(false)
        ...
        <span>
          <Link
            className='btn btn-sm btn-light-primary bg-success text-white fs-6 mb-2 btn btn-rounded'
             to={{pathname:'/master/modular-discount/list',state:{search:state.mainSearch}}}
          >
            Back To List
          </Link>
        </span>
        ...
        <div id='kt_account_profile_details' className='collapse show'>
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            ...
            <div className='card-footer d-flex justify-content-end py-6 px-9'>
              ...
              <Link className='btn btn-danger'  to={{pathname:'/master/modular-discount/list',state:{search:state.mainSearch}}}>
                Cancel
              </Link>
            </div>
          </form>
