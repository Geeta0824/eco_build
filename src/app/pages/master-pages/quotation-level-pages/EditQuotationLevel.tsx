...
import {
  getDesignStageByDesignStageId,
  updateDesignStage,
} from '../../../modules/master-page/design-stage-page/DesignStageCRUD'
...
const EditQuotationLevel: React.FC = () => {
  const {quotationLevelID} = useParams<{quotationLevelID: string}>()
  ...
  function getDesignStageDataByID(mainSearch: string) {
    getDesignStageByDesignStageId(parseInt(quotationLevelID))
      .then((response) => {
  ...
  const formik = useFormik<IQuotationLevelModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        updateDesignStage(
          parseInt(quotationLevelID),
          values.quotationLevelName,
          user.employeeID,
          '192.168.0.1'
        )
          .then((response) => {
      ...
      <div className='text-end'>
        <span>
          <Link
            ...
            to={{pathname: '/master/quotation-level/list', state: {search: state.mainSearch}}}
          >
            Back To List
          </Link>
        </span>
...
export default EditQuotationLevel
