...
import {
  BookedCustomQuotationApi,
  GetCustomizationQuotationListApi,
  customizationQuotaionDiscountApi,
} from '../../modules/customization-quotation-master-page/CustomizationQuotationsCRUD'
import axios from 'axios'
import {DiscountTypeData, MonthDropdownData} from '../other-dropDowns/otherDropDowns'
...
const CustomizationQuotationsListPage: React.FC<Props> = () => {
  ...
  function getAllDIYQuotationData(
    ...
    GetCustomizationQuotationListApi(selEmployeeID, selCustomerID, searchText)
      .then((response) => {
  ...
  const handleShowBooked = (tmpQuotationID: number) => {
    setState({
      ...state,
      selDIYQuotationID: tmpQuotationID,
      loading: false,
  ...
  function BookedCustomQuotation(temQueID: number) {
    BookedCustomQuotationApi(temQueID, user.employeeID)
      .then((response) => {
        if (response.data.isSuccess == true) {
  ...
  function getQuotationPdf(pathname: string, quotationID: number) {
    var URL = ''
    if (pathname == `/customization-quotations/admin-pdf/${quotationID}`) {
      setAdmindwnldLoader(true)
      setselAdmDwID(quotationID)
      URL = `${process.env.REACT_APP_API_URL}/PDF/DownloadCustomQuotationPDFForAdmin`
    } else if (pathname == `/customization-quotations/agency-breakup-pdf/${quotationID}`) {
      setAbrdwnldLoader(true)
      setselAbrDwID(quotationID)
      URL = `${process.env.REACT_APP_API_URL}/PDF/DownloadCustomQuotationPDFForAdminWithAgency`
    } else if (pathname == `/customization-quotations/pdf/${quotationID}`) {
      setDownloadLoader(true)
      setselDwID(quotationID)
      URL = `${process.env.REACT_APP_API_URL}/PDF/DownloadCustomQuotationPDF`
    } else {
      alert('Error')
    }
    ...
    axios.post(URL, {quotationID: quotationID, isDownload: 1}).then((response) => {
      ...
      aPdfDownload.click()
      aPdfDownload.remove()
      if (pathname == `/customization-quotations/pdf/${quotationID}`) {
        setDownloadLoader(false)
        setselDwID(0)
      } else if (pathname == `/customization-quotations/admin-pdf/${quotationID}`) {
  ...
  return (
    <>
      <div className={`card `}>
        ...
        <div className='card-header border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          >
            <Link
              to={{
                pathname: '/customization-quotations/add',
                state: {
                      ...
                      <tr key={index}>
                          ...
                          {data.isCheckOut === true ? (
                            <>
                              {downloadLoader && selDwID == data.quotationID ? (
                                ...
                                <span
                                  // to={`/customization-quotations/pdf/${data.quotationID}`}
                                  onClick={() =>
                                    getQuotationPdf(
                                      `/customization-quotations/pdf/${data.quotationID}`,
                                      data.quotationID
                                    )
                                  }
                        ...
                        <td className='text-center'>
                          {data.isBooked === true ? (
                            ...
                            <Link
                              to={{
                                pathname: `/customization-quotations/view-cart/${data.quotationID}`,
                                state: {
                                  packageID: 0,
                                  packageTypeID: 2,
                          ...
                          {data.isCheckOut === true ? (
                            <>
                              {admindwnldLoader && selAdmDwID == data.quotationID ? (
                                ...
                                <span
                                  // to={`/customization-quotations/admin-pdf/${data.quotationID}`}
                                  onClick={() =>
                                    getQuotationPdf(
                                      `/customization-quotations/admin-pdf/${data.quotationID}`,
                                      data.quotationID
                                    )
                                  }
                                  className='btn btn-icon btn-bg-light bg-hover-primary text-hover-inverse-primary btn-sm me-1 text-primary text-hover-light'
                          ...
                          {data.isCheckOut === true ? (
                            <>
                              {abrDwnldLoader && selAbrDwID == data.quotationID ? (
                                ...
                                <span
                                  // to={`/customization-quotations/agency-breakup-pdf/${data.quotationID}`}
                                  onClick={() =>
                                    getQuotationPdf(
                                      `/customization-quotations/agency-breakup-pdf/${data.quotationID}`,
                                      data.quotationID
                                    )
                                  }
                                  className='btn btn-icon btn-bg-light bg-hover-primary text-hover-inverse-primary btn-sm me-1 text-primary text-hover-light'
      ...
      <Modal show={showBooked} onHide={handleCloseBooked} backdrop='true' keyboard={false}>
        <Modal.Header closeButton>
          <Modal.Title>Booked Custom Quotation</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default CustomizationQuotationsListPage
