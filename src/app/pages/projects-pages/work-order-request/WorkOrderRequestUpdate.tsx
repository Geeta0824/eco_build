...
import {
  ...
  GetWorkOrderListByWorkOrderRequestIDApi,
  UpdateWorkOrderRequestList_PMC_API,
  UpdateWorkOrderRequestListAPI,
...
const WorkOrderRequestUpdate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getWorkOrderListByWorkOrderRequestID(mainSearch)
    }, 100)
  }, [])

  function getWorkOrderListByWorkOrderRequestID(mainSearch: string) {
    GetWorkOrderListByWorkOrderRequestIDApi(parseInt(workOrderID))
      .then((response) => {
        ...
        formik.setFieldValue('description', responseData.remarks)
        formik.setFieldValue('companyName', responseData.companyName)
        formik.setFieldValue('projectName', responseData.projectName)
        formik.setFieldValue('workCompleteDate', responseData.workCompleteDate)
        formik.setFieldValue('assignDate', responseData.assignDate)
        setProjectFilePath(responseData.filePath)
        getProjectStatusDataList(responseData.projectID, responseData.vendorTypeID, mainSearch)
      })
      .catch((error) => {
  ...
  function getProjectStatusDataList(
    temProjectID: number,
    temVendorTypeID: number,
    mainSearch: string
  ) {
    GetPMCStageList_ByProjectIDApi(temProjectID)
      .then((response) => {
                      ...
                      <div className='row mb-6'>
                        ...
                        >
                          <div className='symbol symbol-45px me-5 cursor-pointer'>
                            <img src={process.env.REACT_APP_API_URL + projectFilePath} alt='img' />
                ...
                <tbody className="border-bottom">
                  {currentPostsVender.length > 0 &&
                    currentPostsVender.map((data, index) => {
                      return (
                        <tr key={index} onClick={() => selectVendor(data)}>
                          <td>
