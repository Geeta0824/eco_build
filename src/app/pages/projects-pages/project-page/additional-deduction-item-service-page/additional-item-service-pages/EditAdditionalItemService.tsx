...
import {
  EditAdditionalItemDetailsAPI,
  getAdditionalItemByAdditionalItemIdAPI,
...
const EditAdditionalItemService: React.FC = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      getPayStructureDataByID(projectID, projName, customerName)
    }, 100)
  }, [])

  const user: UserModel = useSelector<RootState>(({auth}) => auth.user, shallowEqual) as UserModel

  function getPayStructureDataByID(projectID: number, projName: string, customerName: string) {
    getAdditionalItemByAdditionalItemIdAPI(parseInt(projectAdditionalItemID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          formik.setFieldValue('additionalItemDescription', response.data.additionalItemDescription)
          formik.setFieldValue('additionalAmount', response.data.additionalAmount)
  ...
  const formik = useFormik<IAdditionalItemServiceModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        EditAdditionalItemDetailsAPI(
          state.projectID,
          parseInt(projectAdditionalItemID),
...
export default EditAdditionalItemService
