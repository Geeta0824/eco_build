...
const ModelPopUpEmployeeMap: React.FC<Props> = ({
  show,
  handleClose,
  EmployeeMapData,
  ProjectID,
  ...
  const [selectedRole, setSelectedRole] = useState<string>('All') // Role selection state with default value 'All'
  const [loading, setLoading] = useState<boolean>(true) // Loading state

  // =================== For Accessories ==========================
  function UserMapForDocument(tech: IEmployeeMapModel[]) {
    let tmpTech = tech
  ...
  function addDocCategoryByDocumentCtgryMstID(technoIds: string) {
    addEmployeeByProjectIDApi(ProjectID, technoIds)
      .then((response) => {
  ...
  const filteredEmployeeData = EmployeeMapData.filter((item) => {
    const matchesSearchQuery = item.employeeName.toLowerCase().includes(searchQuery.toLowerCase())
    const matchesRole = selectedRole === 'All' || item.designationName === selectedRole // Role filter logic
    ...
    <Modal size='xl' show={show} onHide={handleClose} keyboard={false}>
      <Modal.Header closeButton style={{backgroundColor: '#2a3952'}}>
        ...
        <h3 className='card-title align-items-start'>
          <span className='text-white card-label fs-5 mb-1'>Project Name: </span>
          <span className='text-primary card-label fw-bolder fs-5 mb-1'> {ProjectName}</span>
        </h3>
      </Modal.Header>
      <Modal.Body>
