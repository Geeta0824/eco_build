...
import {
  GetProjectProject3dImageByImageByID,
  updateProjectProject3dImageByProjectID,
} from '../../../../modules/project-master-page/project-master/_3d-photos-master-pages/_3DPhotosCRUD'

const profileDetailsSchema = Yup.object().shape({
  photoTitle: Yup.string().required('Title Feild is required'),
})
...
const Edit3DPhotosPage: React.FC = () => {
  const [data, setData] = useState<I3dImagesModel>(initialValues)
  const [fileLoader, setFileLoader] = useState(false)
  const {projectImageID} = useParams<{projectImageID: string}>()
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      getProject3dImageDataByID(projectID, projName, customerName)
    }, 100)
  }, [])

  const user: UserModel = useSelector<RootState>(({auth}) => auth.user, shallowEqual) as UserModel

  function getProject3dImageDataByID(projectID: number, projName: string, customerName: string) {
    GetProjectProject3dImageByImageByID(parseInt(projectImageID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          formik.setFieldValue('photoTitle', response.data.photoTitle)
          formik.setFieldValue('filePath', response.data.photoPath)
  ...
  const imageUpload = (e: any) => {
    ...
    fetch(
      process.env.REACT_APP_API_URL +
        `/ProjectImage/UploadProjectPhotoByProjectID/${state.projectID}`,
      {
        method: 'POST',
        body: formData,
      }
...
export default Edit3DPhotosPage
