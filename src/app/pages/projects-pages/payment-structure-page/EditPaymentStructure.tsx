...
import { GetPrjPayStructuretByID, UpdateProjectPaymentStructureByProjectID } from '../../../modules/project-master-page/payment-structure-master-page/PaymentStructureCRUD'
...
const profileDetailsSchema = Yup.object().shape({
  ...
  amtPercentage: Yup.number()
    ...
    .min(1, 'Amount Percentage is required'),
  stageName: Yup.string().required('Stage Name is required'),
})

interface ITurnkey {
  loading: boolean
  PaymentStructureData: IPaymentStructureModel[]
  tmpPaymentStructureData: IPaymentStructureModel[]
  searchText: string
  turnkeyProjPaymentStageID: number
}

const EditPaymentStructure: React.FC = () => {
  const {projectPaymentStructureID} = useParams<{projectPaymentStructureID: string}>()
  ...
  const [state, setState] = useState<ITurnkey>({
    loading: false,
    PaymentStructureData: [] as IPaymentStructureModel[],
    tmpPaymentStructureData: [] as IPaymentStructureModel[],
    searchText: '',
  ...
  function getPayStructureDataByID() {
    GetPrjPayStructuretByID(parseInt(projectPaymentStructureID))
      .then((response) => {
  ...
  const formik = useFormik<IPaymentStructureModel>({
    ...
    onSubmit: (values) => {
      setLoading(true)
      setTimeout(() => {
        UpdateProjectPaymentStructureByProjectID(
          parseInt(projectPaymentStructureID),
          values.sequenceNo,
          values.stageName,
...
export default EditPaymentStructure
