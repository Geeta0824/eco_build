...
interface IDepartment {
  ...
  agencyRemarkData: IAgencyRemarksModel[]
...
const EditAgencyRemarks: React.FC = () => {
  const location = useLocation()
  const [data, setData] = useState<IAgencyRemarksModel>(initialValues)
  const [isActive, setIsActive] = useState(true)
  const {agencyRemarksID} = useParams<{agencyRemarksID: string}>()
  ...
  const [state, setState] = useState<IDepartment>({
    ...
    agencyRemarkData: [] as IAgencyRemarksModel[],
    ...
    RemarksTypeGetListApi()
      .then((response) => {
        ...
        if (response.data.isSuccess === true) {
          ...
          //   loading: false,
          // })
          AgencyRemarksDataID(agencyTypeData, responseData, mainSearch)
        } else {
          toast.error(`${response.data.message}`)
  ...
  function AgencyRemarksDataID(
    agencyTypeData: IAgencyTypeModel[],
    remarksTypeData: IRemarkTypeModel[],
    mainSearch: string
  ) {
    getAgencyRemarksApi(parseInt(agencyRemarksID))
      .then((response) => {
        if (response.data.isSuccess == true) {
          formik.setFieldValue('remarks', response.data.remarks)
          formik.setFieldValue('agencyTypeID', response.data.agencyTypeID)
          formik.setFieldValue('remarksTypeID', response.data.remarksTypeID)
          setIsActive(response.data.isActive)
          setState({
            ...state,
            selAgencyTypeID: response.data.agencyTypeID,
            selRemarksTypeID: response.data.remarksTypeID,
            agencyTypeData: agencyTypeData,
            remarksTypeData: remarksTypeData,
            mainSearch,
            loading: false,
          })
        } else {
  ...
  const formik = useFormik<IAgencyRemarksModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        updateAgencyRemarksApi(
          // values.agencyRemarksID,
          parseInt(agencyRemarksID),
          values.agencyTypeID,
          values.remarksTypeID,
          values.remarks,
          isActive
        )
          .then((response) => {
...
export default EditAgencyRemarks
