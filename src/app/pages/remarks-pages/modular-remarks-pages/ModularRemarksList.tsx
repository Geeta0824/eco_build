...
import {
  ...
  getAllCountry,
  isActiveCountry,
} from '../../../modules/master-page/country-master-page/CountryCRUD'
import {Pagination} from 'antd'
import LoaderInTable from '../../common-pages/LoaderInTable'
import BlankDataImageInTable from '../../common-pages/BlankDataImageInTable'
import {ModelPopUpDelete} from '../../common-pages/ModelPopUpDelete'
import { IModularRemarksModel} from '../../../models/remarks-page/IModularRemarksModel'
import { deleteModularRemarkApi, getAllModularRemarkApi, UpdateQuotationRemarksIsactive} from '../../../modules/remarks-master-pages/modular-remarks-master-page/ModularRemarksCRUD'
import { ModelPopUpIsActive } from '../../common-pages/ModelPopUpIsActive'

interface ICountry {
  loading: boolean
  modularRemarksData: IModularRemarksModel[]
  tmpModularRemarksData: IModularRemarksModel[]
  imageShow: string
  SearchText: string
  mainSearch: string
...
const ModularRemarksList: React.FC<Props> = () => {
  const location=useLocation()
  const [state, setState] = useState<ICountry>({
    loading: false,
    modularRemarksData: [] as IModularRemarksModel[],
    tmpModularRemarksData: [] as IModularRemarksModel[],
    imageShow: '',
    SearchText: '',
    mainSearch: '',
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getmodularRemarksData(mainSearch)
    }, 100)
  }, [])

  // ==================== Get Country API Call===================

  function getmodularRemarksData(mainSearch:string) {
    getAllModularRemarkApi()
      .then((response) => {
        ...
        if (response.data.isSuccess == true) {
          if (mainSearch !== '') {
            ...
            setState({
              ...state,
              modularRemarksData: results,
              tmpModularRemarksData: responseData,
              loading: false,
            })
            ...
            setState({
              ...state,
              modularRemarksData: responseData,
              tmpModularRemarksData: responseData,
              loading: false,
            })
          ...
          setState({...state, modularRemarksData: [], loading: false})
        }
      })
      .catch((error) => {
        toast.error(`${error}`)
...
const [showActive, setShowActive] = useState(false)
  ...
  function checkedFunction(temEmpId: number, temIsAct: boolean) {
    UpdateQuotationRemarksIsactive(temEmpId, temIsAct)
      .then((response) => {
        if (response.data.isSuccess == true) {
          getmodularRemarksData(state.mainSearch)
          setShowActive(false)
        } else {
  ...
  const deleteExpenseTypeItem = (quotationRemarksID: number) => {
    deleteModularRemarkApi(quotationRemarksID)
      .then((response) => {
        if (response.data.isSuccess === true) {
  ...
  const filter = (e: any) => {
    ...
    if (keyword !== '') {
      const results = state.tmpModularRemarksData.filter((user) => {
        return (
      ...
      setState({...state, modularRemarksData: results, loading: false})
      setTotal(results.length)
      setPage(1)
    } else {
      setState({...state, modularRemarksData: state.tmpModularRemarksData, loading: false})
      // If the text field is empty, show all users
      setTotal(state.tmpModularRemarksData.length)
      setPage(1)
    }
  ...
  const currentPosts: IModularRemarksModel[] = state.modularRemarksData.slice(
    indexOfFirstPage,
    indexOfLastPage
  )
    ...
    <>
      <div className={`card `}>
        ...
        <div className='card-header border-0 py-2 bg-dark'>
          ...
          >
            <Link
              to={{pathname:'/remarks/modular-rmk/add',state:{mainSearch:name}}}
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <div className='d-flex justify-content-end flex-shrink-0'>
                            <Link
                              to={{pathname:`/remarks/modular-rmk/edit/${data.quotationRemarksID}`,state:{mainSearch:name}}}
...
export default ModularRemarksList
