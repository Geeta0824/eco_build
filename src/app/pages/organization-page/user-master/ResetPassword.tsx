...
import {UserPasswordReset} from '../../../modules/change-password/ChangePasswordCRUD'
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        if (values.newPassWord === values.conformNewPassWord) {
          UserPasswordReset(parseInt(userid), values.newPassWord)
            .then((resp) => {
              if (resp.data.isSuccess == true) {
                toast.success('Password Reset Successfully')

                history.push({pathname: '/organization/user/list', state: {search: searchText}})
                setLoading(false)
              } else {
