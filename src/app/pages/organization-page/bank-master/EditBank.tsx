...
import {
  getOrganizationBankByOrganizationBankID,
...
const EditBank: React.FC = () => {
  ...
  const {organisationBankID} = useParams<{organisationBankID: string}>()
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      getOrganizationBankDataByOrganizationBankID(mainSearch)
    }, 100)
  }, [])
  ...
  function getOrganizationBankDataByOrganizationBankID(mainSearch: string) {
    getOrganizationBankByOrganizationBankID(parseInt(organisationBankID)).then((response) => {
      const resData = response.data
      formik.setFieldValue('bankAccountTypeID', resData.bankAccountTypeID)
      formik.setFieldValue('bankName', resData.bankName)
      formik.setFieldValue('branchName', resData.branchName)
      formik.setFieldValue('accountNumber', resData.accountNumber)
      formik.setFieldValue('accountHolderName', resData.accountHolderName)
      formik.setFieldValue('ifscCode', resData.ifscCode)
      formik.setFieldValue('micrCode', resData.micrCode)
      setIsActive(resData.isActive)
      setState({...state, selAccountTypeId: resData.bankAccountTypeID, mainSearch})
    })
  }
        ...
        updateOrganizationBank(
          ...
          .then((response) => {
            if (response.data.isSuccess === true) {
              ...
              history.push({pathname: '/organization/bank/list', state: {search: state.mainSearch}})
            } else {
          ...
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
                      ...
                      bankAccountTypeData.map((data, index) => {
                        return (
                          <option
                            ...
                            selected={
                              state.selAccountTypeId == data.bankAccountTypeID ? true : false
                            }
                          >
            ...
            <div className='card-footer d-flex justify-content-end py-6 px-9'>
              ...
              <Link
                ...
                to={{pathname: '/organization/bank/list', state: {search: state.mainSearch}}}
              >
                Cancel
              </Link>
            </div>
