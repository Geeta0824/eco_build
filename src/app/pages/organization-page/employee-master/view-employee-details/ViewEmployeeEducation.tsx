...
interface IMyProfile {
  ...
  selEduID: IEmployeeEducationModel
}

export function ViewEmployeeEducation() {
  const {employeeID} = useParams<{employeeID: string}>()
  const user: UserModel = useSelector<RootState>(({auth}) => auth.user, shallowEqual) as UserModel
  const [state, setState] = useState<IMyProfile>({
    loading: false,
    empEduData: [] as IEmployeeEducationModel[],
    selEduID: {} as IEmployeeEducationModel,
  })
  ...
  function handleShow(Data: IEmployeeEducationModel) {
    getEmpEducationById(`${Data.employeeEducationID}`)
      .then((response) => {
        setState({
          ...state,
          selEduID: response.data,
        })
      })
      .catch((error) => {
        ...
        <Modal.Body>
          <div className='card-body pt-2'>
            ...
            <div className='row g-4'>
              ...
              <span className='col-9 col-sm-9 fw-bold fs-4'>{state.selEduID.eduCategoryName}</span>
            </div>
            <div className='w-100 border-bottom d-md-block my-2'></div>
            <div className='row g-4'>
              <span className='col-3 col-sm-3 text-gray-800 text-hover-primary fw-bold'>
              ...
              <span className='col-9 col-sm-9 fw-bold fs-4'>{state.selEduID.instituteName}</span>
            </div>
            <div className='w-100 border-bottom d-md-block my-2'></div>
            <div className='row g-4'>
              <span className='col-3 col-sm-3 text-gray-800 text-hover-primary fw-bold'>
              ...
              <span className='col-9 col-sm-9 fw-bold fs-4'>{state.selEduID.subjectName}</span>
            </div>
            <div className='w-100 border-bottom d-md-block my-2'></div>
            <div className='row g-4'>
              <span className='col-3 col-sm-3 text-gray-800 text-hover-primary fw-bold'>
              ...
              <span className='col-9 col-sm-9 fw-bold fs-4'>{state.selEduID.passingYear}</span>
            </div>
            <div className='w-100 border-bottom d-md-block my-2'></div>
            <div className='row g-4'>
              <span className='col-3 col-sm-3 text-gray-800 text-hover-primary fw-bold'>
              ...
              <span className='col-9 col-sm-9 fw-bold fs-4'>{state.selEduID.percentage}%</span>
            </div>
            <div className='w-100 border-bottom d-md-block my-2'></div>
          </div>
        </Modal.Body>
