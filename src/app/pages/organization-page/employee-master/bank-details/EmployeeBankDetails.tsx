...
interface IEmpBankDetails {
  ...
  tmpEmpBankData: IEmployeeBankDetailsModel[]
  selEmpBankData: IEmployeeBankDetailsModel
  selEmpID: number
  SearchText: string
  mainSearch: string
  selBankId: number
...
const EmployeeBankDetails: React.FC<Props> = () => {
  ...
  const [state, setState] = useState<IEmpBankDetails>({
    ...
    tmpEmpBankData: [] as IEmployeeBankDetailsModel[],
    selEmpBankData: {} as IEmployeeBankDetailsModel,
    selEmpID: 0,
    SearchText: '',
    mainSearch: '',
    selBankId: 0,
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getAllEmpBankDetailsDataByEmpID(mainSearch)
    }, 100)
  }, [])

  function getAllEmpBankDetailsDataByEmpID(mainSearch: string) {
  ...
  function checkedFunction(temEmpBankId: number, temEmpBankIsAct: boolean) {
    isActiveEmpBankDetails(temEmpBankId, temEmpBankIsAct)
      .then((response) => {
        if (response.data.isSuccess === true) {
          getAllEmpBankDetailsDataByEmpID(state.mainSearch)
          setShowActive(false)
        } else {
          toast.error(`${response.data.message}`)
        }
  ...
  const handleShow = (employeeBankID: number) => {
    setState({
      ...state,
      selBankId: employeeBankID,
      loading: false,
    })
    setShow(true)
  }

  function deleteEmpBankDetail(empBankId: number) {
  ...
  const filter = (e: any) => {
    ...
    if (keyword !== '') {
      const results = state.tmpEmpBankData.filter((user) => {
        return (
      ...
      setState({...state, empBankData: results})
      setTotal(results.length)
      setPage(1)
    } else {
      setState({...state, empBankData: state.tmpEmpBankData})
      // If the text field is empty, show all users
      setTotal(state.tmpEmpBankData.length)
      setPage(1)
    }
      ...
      <div className={`card `}>
        ...
        <div className='card-header border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          >
            <Link
              to={{
                pathname: `/organization/employee/edit/${parseInt(employeeID)}/bank/add`,
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <div className='d-flex justify-content-end flex-shrink-0'>
                            ...
                            <Link
                              to={{
                                pathname: `/organization/employee/edit/${parseInt(
                                  employeeID
                                )}/bank/edit/${data.employeeBankID}`,
                                state: {mainSearchBank: name},
                              }}
                            ...
                            <div
                              onClick={(e) => handleShow(data.employeeBankID)}
