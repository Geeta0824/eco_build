...
import {
  IEmployeeBankDetailsModel,
  employeeBankDetailsIniValue as initialValues,
} from '../../../../models/organization-page/Employee/IEmployeeBankDetailsModel'
import {UserModel} from '../../../../modules/auth/models/UserModel'
import {RootState} from '../../../../../setup'
import {
  getEmpBankDetByEmpId,
  updateEmpBankDetails,
...
const EditEmployeeBank: React.FC = () => {
  ...
  const [data, setData] = useState<IEmployeeBankDetailsModel>(initialValues)
  const updateData = (fieldsToUpdate: Partial<IEmployeeBankDetailsModel>): void => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      getEmpBankDataByBankEmpId(mainSearch)
    }, 100)
  }, [])

  function getEmpBankDataByBankEmpId(mainSearch: string) {
    getEmpBankDetByEmpId(bankID)
      .then((response) => {
        ...
        formik.setFieldValue('accountTypeID', response.data.accountTypeID)
        formik.setFieldValue('pfaCompanyName', response.data.pfaCompanyName)
        formik.setFieldValue('pfaunNumber', response.data.pfaunNumber)
        formik.setFieldValue('pfaNumber', response.data.pfaNumber)
        formik.setFieldValue('esicNumber', response.data.esicNumber)
        formik.setFieldValue('esicStartDate', response.data.esicStartDate)
        formik.setFieldValue('esicExpDate', response.data.esicExpDate)
        formik.setFieldValue('pfaStartDate', response.data.pfaStartDate)
        formik.setFieldValue('pfaEndDate', response.data.pfaEndDate)
        setIsActive(response.data.isActive)
        setState({
          ...state,
          loading: false,
          selEmpID: parseInt(employeeID),
          selAccTypeID: response.data.accountTypeID,
          mainSearch,
        })
      })
      .catch((error) => {
  ...
  const formik = useFormik<IEmployeeBankDetailsModel>({
    initialValues,
        ...
        if (Edit) {
          updateEmpBankDetails(
            ...
            .then((response) => {
              ...
              history.push({pathname:`/organization/employee/edit/${state.selEmpID}/bank/list`,state:{search:state.mainSearch}})
              setLoading(false)
            })
            .catch((error) => {
              toast.error(`${error}`)
    ...
    <>
      ...
      >
        <div className='card-title m-0'>
          <h3 className='fw-bolder m-0'>Update Employee Bank Details </h3>
        </div>
      </div> */}
        <div id='kt_account_profile_details' className='collapse show'>
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            ...
            <div className='card-footer d-flex justify-content-end py-6 px-9'>
              ...
              <Link
                to={{
                  pathname: `/organization/employee/edit/${state.selEmpID}/bank/list`,
