...
import {createEmpDocMap} from '../../../../modules/organization-page/employee-master-page/document-details/EmpDocMapCRUD'
...
const AddEmployeeDocument: React.FC = () => {
  ...
  function getDocumentTypeData(finalempID: number, mainSearch: string) {
    getDocumentType()
      .then((response) => {
        ...
        getMediaTypeData(responseData, finalempID, mainSearch)
      })
      .catch((error) => {
        toast.error(`${error}`)
  ...
  const fileUpload = (e: any) => {
    ...
    if (state.selMediaType === 0) {
      ...
      fetch(
        process.env.REACT_APP_API_URL + `/EmployeeDocMap/uploadEmployeeDocument/${state.selEmpID}`,
        {
          method: 'POST',
          body: formData,
        }
