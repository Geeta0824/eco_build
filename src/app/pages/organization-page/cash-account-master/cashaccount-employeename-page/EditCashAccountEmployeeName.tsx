...
import {
  ...
  GetCashAccountSegmentByID,
  UpdateCashSegmentAccount,
...
const EditCashAccountEmployeeName: React.FC = () => {
  ...
  const {cashEmployeeBalanceID} = useParams<{cashEmployeeBalanceID: string}>()
  const [isActive, setIsActive] = useState(true)
  const [cashAccouID, setcashAccouID] = useState(0)
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setcashAccouID(cashAccouID)
      setemployeeID(employeeID)
      var mainSearch: string = ''
      if (lc.mainSearch !== undefined) {
        mainSearch = lc.mainSearch
      }
      // getAllEmpBankDetailsDataByEmpID(employeeID, cashAccouID)
      getCashAccountSegmentByIDData(mainSearch)
    }, 100)
  }, [])

  function getCashAccountSegmentByIDData(mainSearch: string) {
    GetCashAccountSegmentByID(parseInt(cashEmployeeBalanceID))
      .then((response) => {
        const resData = response.data
        formik.setFieldValue('accountBalance', resData.accountBalance)
        formik.setFieldValue('cashEmployeeSubTypeID', resData.cashEmployeeSubTypeID)
        formik.setFieldValue('employeeBankID', resData.employeeBankID)


        getAllEmpBankDetailsDataByEmpID(
          resData.emplyeeID,
          resData.cashAccouID,
          resData.cashEmployeeSubTypeID,
          resData.employeeBankID,
          mainSearch
        )
      })
      .catch((error) => {
      ...
      .then((response) => {
        if (response.data.isSuccess === true) {
          ...
          setState({
            ...
            selEmployeeSubTypeID: cashEmployeeSubTypeID,
            selEmployeeBankID: employeeBankID,
            mainSearch,
            // loading: false,
          })
  ...
  const formik = useFormik<ICashAccountModel>({
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        UpdateCashSegmentAccount(
          parseInt(cashEmployeeBalanceID),
          parseInt(cashAccoID),
          ...
          <form onSubmit={formik.handleSubmit} noValidate className='form'>
            ...
            <div className='card-footer d-flex justify-content-end py-9'>
              ...
              <Link
                ...
                // to={{
                //   pathname:
                //     '/organization/cashaccount/cashaccountemployeename/${data.cashAccountID}/list',
...
export {EditCashAccountEmployeeName}
