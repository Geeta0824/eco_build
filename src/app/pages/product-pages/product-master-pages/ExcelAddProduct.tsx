...
const ExcelAddProduct: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (lc.mainProductCategoryID !== undefined) {
        mainProductCategoryID = lc.mainProductCategoryID
        mainUnitID = lc.mainUnitID
        mainSearch = lc.mainSearch
      }
      getState(mainProductCategoryID, mainUnitID, mainSearch)
    }, 100)
  }, [])

  function getState(mainProductCategoryID: number, mainUnitID: number, mainSearch: string) {
    setState({
      ...state,
  ...
  const handleSubmit = () => {
    ...
    fetch(process.env.REACT_APP_API_URL + '/Product/UploadProductExcel', {
      method: 'POST',
      body: formData,
    })
      .then((res) => res.json())
      .then((response) => {
        if (response.isSuccess === true) {
          ...
          history.push({
            ...
            state: {ProductCategoryID: state.mainProductCategoryID, search: state.mainSearch},
          })
...
export default ExcelAddProduct
