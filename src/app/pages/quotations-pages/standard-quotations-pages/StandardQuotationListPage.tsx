...
import {IStandardQuotationsModel} from '../../../models/quotations-page/standard-quotation-page/IStandardQuotationsModel'
...
interface IStandard {
  loading: boolean
  standardQuotationsData: IStandardQuotationsModel[]
  tmpStandardQuotationsData: IStandardQuotationsModel[]
...
const StandardQuotationListPage: React.FC<Props> = () => {
  ...
  const [state, setState] = useState<IStandard>({
    loading: false,
    standardQuotationsData: [] as IStandardQuotationsModel[],
    tmpStandardQuotationsData: [] as IStandardQuotationsModel[],
  ...
  function getAllstandardQuotationsData(
    ...
    GetStandarQuotationList(selEmployeeID, selCustomerID, searchText)
      .then((response) => {
        if (response.data.isSuccess == true) {
          ...
          setState({
            ...
            standardQuotationsData: responseData,
            tmpStandardQuotationsData: responseData,
            loading: false,
          })
          setTotal(responseData.length)
        } else {
          toast.error(`${response.data.message}`)
          setState({...state, standardQuotationsData: [], loading: false})
        }
      })
      .catch((error) => {
        toast.error(`${error}`)
 ...
 //  console.log(keyword)
  ...
  const [total, setTotal] = useState(state.standardQuotationsData.length) //  length
  const [page, setPage] = useState(1)
  const [postPerPage, setPostPerPage] = useState(10)
  const indexOfLastPage = page * postPerPage
  const indexOfFirstPage = indexOfLastPage - postPerPage
  const currentPosts: IStandardQuotationsModel[] = state.standardQuotationsData.slice(
    indexOfFirstPage,
    indexOfLastPage
  )
    ...
    <>
      <div className={`card `}>
        ...
        <div className='card-header border-0 py-2 ' style={{backgroundColor: '#000000'}}>
          ...
          >
            <Link
              to='/quotations/standards-quotations/add'
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className='text-center'>
                          <Link
                            to={`/quotations/standards-quotations/view/${data.quotationID}`}
                        ...
                        {/* <td className='text-center'>
                          <span
                            // to={`/organization/employee/view/${data.standardQuotationID}/personal`}
                            className='btn btn-icon btn-bg-light bg-hover-primary text-hover-inverse-primary btn-sm me-1 text-primary text-hover-light'
                            data-bs-toggle='tooltip'
                            data-bs-placement='top'
                            title='View'
